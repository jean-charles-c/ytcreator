...
export const STYLE_SUFFIXES: Record<string, StyleDef> = {
  ...
  invertedlineart: {
    ...
    promptSuffix: "Pure black background only, RGB (0,0,0), no grey, no off-black, no dark tones whatsoever. All drawn elements rendered exclusively in white or near-white strokes. Absolutely no color, no grey fill, no shading gradient of any kind. Line style must mimic white chalk or white pencil on blackboard. Strokes are slightly irregular, hand-drawn, with subtle texture and natural pressure variation. Lines are never perfectly smooth or vector-like. No clean digital edges. Characters and objects are defined by outline contours only. No interior fill of any kind — neither white nor grey. Solid black volumes are strictly forbidden except where the black background itself acts as shadow or negative space. The body, face, and clothing of characters must remain transparent to the black background — only their contour is drawn. Shading and texture are achieved exclusively through loose hatching, cross-hatching or repeated parallel chalk strokes, never through grey fills or gradient overlays. No speech bubbles, no text, no UI elements unless explicitly requested. The overall rendering must look like a frame from a hand-animated chalk-on-blackboard sequence. Mood is intimate, raw, slightly melancholic. High contrast. Zero digital polish.",
    promptSuffixFr: "Style : fond noir pur uniquement, RGB (0,0,0), aucun gris, aucun noir atténué. Tous les éléments dessinés exclusivement en traits blancs ou quasi-blancs. Aucune couleur, aucun remplissage gris, aucun dégradé. Le trait imite la craie blanche ou le crayon blanc sur tableau noir. Traits légèrement irréguliers, tracés à la main, avec texture subtile et variation de pression naturelle. Personnages et objets définis par contours uniquement, aucun remplissage intérieur. L'ombrage est obtenu exclusivement par hachures libres ou croisées, jamais par remplissages gris. Aucune bulle de texte ni élément d'interface. Rendu global évoquant une séquence animée craie-sur-tableau-noir. Ambiance intime, brute, légèrement mélancolique. Contraste élevé. Zéro finition numérique.",
  },
};
