...
export interface ExactShotSyncValidationResult {
  ok: boolean;
  errors: string[];
...
export function validateExactShotSentences(
  expectedShotIds: string[],
  shotSentences: ExactShotSentenceLike[] | null | undefined
): ExactShotSyncValidationResult {
  const entries = shotSentences ?? [];
