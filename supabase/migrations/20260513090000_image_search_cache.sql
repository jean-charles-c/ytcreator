...
-- Stores validated image results keyed by SHA-256(nom|epoque|description).
-- Accessed only by the Edge Function via service_role; no public RLS policy.
...
-- No PUBLIC policies on purpose: only service_role (Edge Function) reads/writes.
