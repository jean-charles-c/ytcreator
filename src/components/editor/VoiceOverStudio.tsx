...
export default function VoiceOverStudio({ narration, generatedScript, projectId, projectTitle, scenes, shots, scenesForSort, onMusicSelected }: VoiceOverStudioProps) {
  ...
  const handleGenerateAllScenes = async () => {
    ...
    if (!freeMode) {
      toast.info(
        "Audio prêt. Cliquez sur « Aligner Whisper » dans l'historique pour lancer l'alignement quand l'audio est validé.",
        { duration: 6000 }
      );
    }
    ...
    try {
      ...
      if (pipelineMode === "chirp3hd") {
        try {
          ...
          // ── Whisper alignment is now manual ──
          if (!freeMode) {
            toast.info(
              "Audio prêt. Cliquez sur « Aligner Whisper » dans l'historique pour lancer la triple passe quand l'audio est validé.",
              { duration: 6000 }
            );
          }
        } catch (e: any) {
  ...
  const handlePlayFromHistory = (audioUrl: string, fileName: string, duration: number) => {
    return _handlePlayFromHistory(audioUrl, fileName, duration);
  };

  // ── Run Whisper alignment on the latest assembled chirp3hd audio ──
  const handleAlignAllScenes = async () => {
    if (!projectId) return;
    setAligningAll(true);
    try {
      const session = (await supabase.auth.getSession()).data.session;
      ...
      // Fetch the most recent chirp3hd-assembled entry
      ...
      if (!entry) {
        toast.error("Aucun audio assemblé trouvé. Générez d'abord les scènes.");
        return;
      }
      ...
      toast.info("Alignement texte/audio Whisper en cours… (1-2 min)");
      ...
      // Notify WhisperAlignmentEditor to reload
      window.dispatchEvent(
      ...
      console.error("[VoiceOverStudio] Align all error:", e);
  ...
  const _handlePlayFromHistory = (audioUrl: string, fileName: string, duration: number) => {
    // Stop current
    if (audioRef.current) {
